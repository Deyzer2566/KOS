...
enum LoadELFResult loadHeader(FIL* file, struct elf_header* header){
	UINT read;
	FRESULT res = f_read(file, header, sizeof(struct elf_header), &read);
	if(res != FR_OK || read != sizeof(struct elf_header))
		return LOAD_ELF_CANT_READ_ELF_HEADER;
	return LOAD_ELF_OK;
}

enum LoadELFResult checkHeader(struct elf_header* header){
	if(header->e_ident.EI_MAG[0] != 0x7f || header->e_ident.EI_MAG[1] != 0x45 || 
		header->e_ident.EI_MAG[2] != 0x4c || header->e_ident.EI_MAG[3] != 0x46)
		return LOAD_ELF_BAD_SIGNATURE;
	if(header->e_ident.EI_CLASS != 1)
		return LOAD_ELF_BAD_CLASS;
	if(header->e_ident.EI_DATA != 1)
		return LOAD_ELF_BAD_BYTE_ORDER;
	if(header->e_ident.EI_VERSION != 1)
		return LOAD_ELF_BAD_ELF_HEAD_VERSION;
	if(header->e_ident.EI_OSABI != 0)
		return LOAD_ELF_INCOMPATIBLE_OSABI;
	if(header->e_type != 2)
		return LOAD_ELF_NOT_EXECUTABLE_FILE;
	if(header->e_machine != 0x28)
		return LOAD_ELF_WRONG_ARCH;
	if(header->e_version != 1)
		return LOAD_ELF_BAD_ELF_VERSION;
	if(header->e_entry != PROGRAM_ENTRY)
		return LOAD_ELF_WRONG_ENTRY;
	return LOAD_ELF_OK;
}

enum LoadELFResult loadProgramHeader(FIL* file, struct program_header* prog_head){
	UINT read;
	FRESULT res = f_read(file, prog_head, sizeof(struct program_header), &read);
	if(res != FR_OK || read != sizeof(struct program_header))
		return LOAD_ELF_CANT_READ_PROGRAM_HEADER;
	return LOAD_ELF_OK;
}
...
enum LoadELFResult loadProcessFromElfFile(char* filename, struct Process* proc){
	...
	if(loadHeader(&file, &header) == LOAD_ELF_CANT_READ_ELF_HEADER){
		f_close(&file);
		return LOAD_ELF_CANT_READ_ELF_HEADER;
	}
	enum LoadELFResult headerCheckResult = checkHeader(&header);
	if(headerCheckResult != LOAD_ELF_OK){
		f_close(&file);
	...
	for(uint16_t i = 0; i < header.e_phnum; i++){
		...
		enum LoadELFResult loadProgramHeaderResult = loadProgramHeader(&file, &prog_head);
		if(loadProgramHeaderResult != LOAD_ELF_OK)
			return loadProgramHeaderResult;
		if(prog_head.p_type == 1){
			if(prog_head.p_vaddr + prog_head.p_memsz - PROGRAM_ENTRY > PROGRAM_SIZE){
				f_close(&file);
				return LOAD_ELF_TOO_LARGE;
			}
